...
with open('cucumber.csv', 'wb') as data:
    s3.download_fileobj('nexus-public-artifacts', 'test-reporting/shopper-tests/Android/cucumber.csv', data)
    wr = csv.writer(data, dialect='excel')
    wr.writerow(arrayofdata)
with open('cucumber.csv', 'rb') as data:
    s3.upload_fileobj(data, 'nexus-public-artifacts', 'test-reporting/shopper-tests/Android/cucumber.csv')
with open('cucumber.json', 'rb') as data:
    s3.upload_fileobj(data, 'nexus-public-artifacts', 'test-reporting/shopper-tests/Android/cucumber.json')